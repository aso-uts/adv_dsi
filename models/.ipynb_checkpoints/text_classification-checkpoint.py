...
def _setup_datasets(dataset_name, root='.data', ngrams=1, vocab=None, include_unk=False, from_path=None):
    if from_path is None:
        print("Download Files")
        dataset_tar = download_from_url(URLS[dataset_name], root=root)
        extracted_files = extract_archive(dataset_tar)
    else:
        _, _, extracted_files = next(os.walk(from_path))
        extracted_files = [f"{from_path}/{file}" for file in extracted_files]
